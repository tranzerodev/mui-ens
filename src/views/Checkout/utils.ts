...
export const getEventOrdered = (tickets: UpdateEventInput['tickets'], values) => {
  return tickets
    .filter((ticket) => Object.keys(values).includes(ticket?.title))
    .flatMap((ticket) => {
      return {
        title: ticket?.title,
        price: ticket?.price,
        quantity: values[ticket?.title],
...
export const getOrderedPrice = (tickets: UpdateEventInput['tickets'], values) => {
  const ticketPrices: number[] = tickets
    .filter((ticket) => Object.keys(values).includes(ticket?.title))
    .map((ticket) => ticket?.price * values[ticket?.title])

  return ticketPrices.reduce((total, ticketPrice) => total + ticketPrice)
}
...
export const getUpdateTicketsOrdered = (
  ...
  return tickets.map((ticket) => {
    const { ordered, title } = ticket
    const quantity = values[title]
